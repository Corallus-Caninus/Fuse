...
// AS PYTORCH DOES NOT HAVE DECOUPLED WEIGHT DECAY FOR ADAGRAD
// ------------------------------------------------------------------------
#[test]
fn adagrad_decoupled_weight_decay_test() -> Result<()> {
    // Generate some linear data, y = 3.x1 + x2 - 2.
    let w_gen = Tensor::new(&[[3f32, 1.]], &Device::Cpu)?;
    let b_gen = Tensor::new(-2f32, &Device::Cpu)?;
    let gen = Linear::new(w_gen, Some(b_gen));
    ...
    assert_eq!(to_vec2_round(&w, 4)?, &[[0.1483, 0.1450]]);
    assert_eq!(to_vec0_round(&b, 4)?, 0.1465);
    Ok(())
}
