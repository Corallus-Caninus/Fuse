...
use crate::{LossOptimizer, ModelOutcome, Trainable};
...
impl Default for ParamsLBFGS {
    fn default() -> Self {
        Self {
            ...
            history_size: 100,
            line_search: None,
            grad_conv: GradConv::RMSForce(1e-9), //TODO was -7
            step_conv: StepConv::RMSStep(1e-15), //TODO was -9
            weight_decay: None,
        }
    }
}
...
#[derive(Debug)]
pub struct Lbfgs<'a, M: Trainable> {
    pub vars: Vec<Var>,
    model: &'a mut M,
    pub s_hist: VecDeque<(Tensor, Tensor)>,
    pub last_grad: Option<Var>,
    pub next_grad: Option<Var>,
    pub last_step: Option<Var>,
    pub params: ParamsLBFGS,
    pub first: bool,
}
#[derive(Default, Clone)]
pub struct lbfgs_state {
    pub s_hist: VecDeque<(Tensor, Tensor)>,
    pub last_grad: Option<Var>,
    pub next_grad: Option<Var>,
    pub last_step: Option<Var>,
...
//TODO: save s_hist, last_grad, next_grad and last step, also create a way to initialize with first false and these saved values.

impl<'a, M: Trainable> LossOptimizer<'a, M> for Lbfgs<'a, M> {
    type Config = ParamsLBFGS;

    fn new(vs: Vec<Var>, params: Self::Config, model: &'a mut M) -> CResult<Self> {
    ...
    fn backward_step(&mut self, loss: &Tensor) -> CResult<ModelOutcome> {
        ...
        let mut grad = if let Some(this_grad) = &self.next_grad {
        ...
        //TODO: can we just manually set step size here? yes gradient clip this lower bound, for now assume l2 norm clips upper
        match self.params.grad_conv {
            ...
            GradConv::RMSForce(tol) => {
                ...
                {
                    info!("grad converged");
                    return Ok(ModelOutcome::GradConverged(loss.clone(), evals));
                }
            }
        }
        ...
        if let Some(yk) = yk {
            if let Some(step) = &self.last_step {
                self.s_hist
                    .push_back((step.as_tensor().copy()?, yk.copy()?));
                //                    .push_back((step.as_tensor().clone(), yk.clone()));
            }
        }

        let gamma = if let Some((s, y)) = self.s_hist.back().clone() {
            let numr = y
                .unsqueeze(0)?
            ...
            //TODO: if this is >1 will we unsteck a convergence point?
            1.
        };
        ...
        if let Some(ls) = &self.params.line_search {
            match ls {
                ...
                LineSearch::StrongWolfe(c1, c2, tol) => {
                    ...
                    lr = t;
                    println!("LR {}", lr);
                    // q.set_and_free((q.clone().as_tensor() * lr)?)?;
            ...
            q.set(&(q.as_tensor() * lr)?)?; //TODO test
                                            //q.set_and_free((q.clone().into_inner() * lr)?)?; //TODO test

            if let Some(step) = &self.last_step {
                step.set(&q.clone().into_inner())?;
            } else {
                self.last_step = Some(Var::from_tensor(&q)?);
            }
            ...
            match self.params.step_conv {
                StepConv::MinStep(tol) => {
                        ...
                        < tol
                    {
                        ...
                        let next_loss = self.model.loss()?.clone();
                        evals += 1;
...
impl<'a, M: Trainable> Lbfgs<'a, M> {
    pub fn save_state(mut self) -> lbfgs_state {
        let res = lbfgs_state {
            //            s_hist: self
            //                .s_hist
            //                .iter()
            //                .map(|x| (x.0, x.1))
            //                .collect(),
            //            last_step: Some(Var::from_tensor(&self.last_step.unwrap().copy().unwrap()).unwrap()),
            //            last_grad: Some(Var::from_tensor(&self.last_grad.unwrap().copy().unwrap()).unwrap()),
            //            next_grad: Some(Var::from_tensor(&self.next_grad.unwrap().copy().unwrap()).unwrap()),
            s_hist: self.s_hist,
            last_step: self.last_step,
            last_grad: self.last_grad,
            next_grad: self.next_grad,
            lr: self.params.lr,
            first: false,
        };
        //        self.s_hist.clear();
    ...
    pub fn load_state(&mut self, lbfgs_state: lbfgs_state) {
        self.s_hist = lbfgs_state.s_hist;
        self.last_step = lbfgs_state.last_step;
        self.last_grad = lbfgs_state.last_grad;
        self.next_grad = lbfgs_state.next_grad;
        self.params.lr = lbfgs_state.lr;
        self.first = lbfgs_state.first;
    }
}
    ...
    if let Some(wd) = weight_decay {
        ...
        for v in vs {
            if let Some(grad) = grads.get(v) {
                ...
                flat_grads.push(candle::Tensor::zeros(n_elems, v.dtype(), v.device())?);
            }
        }
    }
    candle::Tensor::cat(&flat_grads, 0)
}
...
fn add_grad(vs: &mut Vec<Var>, flat_tensor: &Tensor) -> CResult<()> {
    ...
    for var in vs {
        ...
        let tensor = flat_tensor
            ...
            .reshape(var.shape())?;
        //TODO: minimize here to gradient clip the floor
        //TODO: use where_cond after a ge 1-hot encoding to ensure the gradient is always a certain distance from 0
