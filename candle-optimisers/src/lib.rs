...
pub trait Trainable: Sized {
    ...
    fn loss(&mut self) -> CResult<Tensor>; //, xs: &Tensor, ys: &Tensor
}

//TODO: can we remove model here? we just need Trainable and params vars
/// trait for optimisers like LBFGS that need the ability to calculate the loss
/// and its gradient
pub trait LossOptimizer<'a, M: Trainable>: Sized {
    ...
    fn from_slice(vars: &[&Var], config: Self::Config, model: &'a mut M) -> CResult<Self> {
